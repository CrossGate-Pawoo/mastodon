...
class Pubsubhubbub::DistributionWorker
  ...
  def perform(stream_entry_ids)
    stream_entries = StreamEntry.where(id: stream_entry_ids).includes(:status).reject { |e| e.status.nil? || e.status.hidden? || Pawoo::TimeLimit.from_status(e.status).present? }
